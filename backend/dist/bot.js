...
import { queueWaMessage, getWaChats, getWaMessages, setWaMapping, formatWaChatList, formatWaMessages, } from './whatsapp.js';
import { createTask as dbCreateTask, getAllTasks, deleteTask as dbDeleteTask, pauseTask as dbPauseTask, resumeTask as dbResumeTask, } from './db.js';
...
const waSelectionState = new Map();
function isAuthorised(chatId) {
    if (ALLOWED_CHAT_IDS.length === 0)
        return true; // first-run mode
...
function escapeHtmlSelective(text) {
...
async function handleMessage(ctx, rawText, forceVoiceReply = false) {
...
function storeUsage(chatId, agentId, usage, source) {
    try {
        insertUsage({
            chatId,
...
export function createBot() {
    ...
    bot.command('memory', async (ctx) => {
        ...
        const lines = memories.map(m => `[${m.sector}] (${m.salience.toFixed(2)}) ${m.content.slice(0, 100)}`);
