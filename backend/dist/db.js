...
export function getSession(chatId) {
    const row = getDb().prepare('SELECT session_id FROM sessions WHERE chat_id = ?').get(chatId);
...
// --- Memories ---
export function insertMemory(chatId, content, sector, topicKey) {
...
export function searchMemories(query, chatId, limit = 3) {
...
export function getMemoriesForDisplay(chatId, limit = 10) {
    return getDb().prepare(`
...
// --- Scheduled Tasks ---
export function createTask(id, chatId, prompt, schedule, nextRun) {
    const now = Math.floor(Date.now() / 1000);
    getDb().prepare(`
...
export function updateTaskAfterRun(id, lastResult, nextRun) {
    const now = Math.floor(Date.now() / 1000);
    getDb().prepare(`
...
export function getAllTasks() {
    return getDb().prepare('SELECT id, chat_id, prompt, schedule, next_run, last_run, status FROM scheduled_tasks ORDER BY created_at DESC').all();
}
...
export function getWaMessages(jid, limit = 20) {
    return getDb().prepare(`
...
export function setWaMapping(waJid, telegramChatId) {
    const now = Math.floor(Date.now() / 1000);
    getDb().prepare(`
    INSERT OR REPLACE INTO wa_message_map (wa_jid, telegram_chat_id, created_at) VALUES (?, ?, ?)
  `).run(waJid, telegramChatId, now);
}
export function getTelegramChatForWa(waJid) {
    const row = getDb().prepare('SELECT telegram_chat_id FROM wa_message_map WHERE wa_jid = ?').get(waJid);
    return row?.telegram_chat_id;
}
export function getAllAgents() {
    return getDb().prepare('SELECT * FROM agents ORDER BY is_default DESC, created_at ASC').all();
}
export function getAgent(id) {
    return getDb().prepare('SELECT * FROM agents WHERE id = ?').get(id);
}
export function getDefaultAgent() {
    return getDb().prepare('SELECT * FROM agents WHERE is_default = 1 LIMIT 1').get();
}
...
export function getSessionWithAgent(chatId) {
    return getDb().prepare('SELECT session_id, agent_id FROM sessions WHERE chat_id = ?').get(chatId);
}
export function setSessionAgent(chatId, agentId) {
    return getDb().prepare('UPDATE sessions SET agent_id = ? WHERE chat_id = ?').run(agentId, chatId).changes > 0;
}
export function getAllSessions() {
    return getDb().prepare('SELECT chat_id, session_id, agent_id, updated_at FROM sessions ORDER BY updated_at DESC').all();
}
export function getSkillOverrides() {
    return getDb().prepare('SELECT * FROM skill_overrides').all();
}
export function setSkillOverride(skillPath, enabled) {
    const now = Math.floor(Date.now() / 1000);
    getDb().prepare(`
...
export function insertActivity(eventType, summary, opts) {
    const now = Math.floor(Date.now() / 1000);
    getDb().prepare(`
...
export function getMemoriesByChatId(chatId, limit = 50) {
    return getDb().prepare('SELECT id, content, sector, salience, created_at FROM memories WHERE chat_id = ? ORDER BY salience DESC, accessed_at DESC LIMIT ?').all(chatId, limit);
}
...
export function getRecentUsage(limit = 50) {
    return getDb().prepare('SELECT * FROM usage_log ORDER BY created_at DESC LIMIT ?').all(limit);
}
export function getUsageByChatId(chatId, limit = 50) {
    return getDb().prepare('SELECT * FROM usage_log WHERE chat_id = ? ORDER BY created_at DESC LIMIT ?').all(chatId, limit);
}
...
export function insertChatMessage(chatId, role, content) {
    const now = Math.floor(Date.now() / 1000);
...
export function getChatMessages(chatId, limit = 100) {
    return getDb().prepare('SELECT * FROM chat_messages WHERE chat_id = ? ORDER BY created_at ASC LIMIT ?').all(chatId, limit);
}
